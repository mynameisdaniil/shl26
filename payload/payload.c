/*
 * Authors: JohannesObermaier, Patrick Pedersen, Daniil Sobol
 *
 * Target Firmware Version: 1.5
 *
...
const char DUMP_START_MAGIC[] = {0xFE, 0xED, 0xF0, 0x0D};
...
int main(void)
{
	...
	while (((uintptr_t)addr) < (0x08000000U + 256)) // Try dumping up to 1M. When reaching unimplemented memory, it will cause hard fault and stop.
	{
		writeWord(*addr);
		++addr;
	}
